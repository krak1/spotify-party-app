...
def get_user_tokens(session_id):
    ...
    # print('len====================',len(user_tokens))
    if len(user_tokens) > 0:
        # print("=============\n", user_tokens[0].refresh_token,"++++++++++++++++++++++++")
...
def refresh_spotify_token(session_id, tokens):
    ...
    # print("=============refresh\n", response.get('refresh_token'),"==================================================access", response.get('access_token'))
    access_token = response.get('access_token')
    token_type = response.get('token_type')
    expires_in = response.get('expires_in')
    # refresh_token = response.get('refresh_token')

    update_or_create_user_tokens(session_id=session_id, refresh_token=refresh_token, access_token=access_token, 
                                expires_in=expires_in, token_type=token_type)
    return
...
def execute_spotify_api_request(session_id, endpoint, post_=False, put_=False):
    ...
    try:
        ...
        return {'Error':'Issue At execute spotify api request(song might not be playing)', "response": response}
